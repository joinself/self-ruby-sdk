...
namespace :sources do
  task :generate do
    # Clone sources definition repo
    puts "[ sources:generate ] removeing old sources spec"
    `rm -rf _support/sources/`
    puts "[ sources:generate ] clonning new sources"
    `git clone git@github.com:joinself/sources.git _support/sources`
    puts "[ sources:generate ] generating sources specification"
    json_file = File.open("./_support/sources/sources.json")
...
module SelfSDK
  SOURCE_DATA = #{build_sources(sources_spec)}
end
HEREDOC
